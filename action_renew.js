...
(async () => {
    ...
    console.log(`正在启动带有代理支持的 Chrome...`);
    let context;
    try {
    ...
    // --- 修改点3：用真实浏览器直接测试代理出口IP ---
    if (PROXY_CONFIG) {
        console.log('[代理] 正在验证代理出口 IP...');
        try {
            await page.goto('https://ifconfig.me/ip', { timeout: 15000 });
            const ip = await page.innerText('body');
            console.log(`[代理] 连接成功！当前出口 IP: ${ip.trim()}`);
        } catch (e) {
            console.error(`[代理] 连接失败，请检查 SOCKS5 节点是否存活: ${e.message}`);
            await context.close();
            process.exit(1);
        }
    }
